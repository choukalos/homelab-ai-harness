...
- [ ] TODO 1 — Phase 6: Build MCP Search Server (search_web, search_recent, search_news)
- [ ] TODO 2 — Phase 7: Build Knowledge MCP Server (kb_search, kb_get_document, kb_list_collections, kb_recent_changes)
- [ ] TODO 3 — Phase 8: Build Skill Runner Skeleton (FastAPI, port 8091, job lifecycle)
...
- [ ] TODO 7 — Phase 10: Draft LiteLLM MCP Config (mcp-search, tool-bundles, model-aliases)
- [ ] TODO 8 — Phase 10: Add Manual Task for MCP Tool Registration to thor_manual_tasks.md
- [ ] TODO 9 — Phase 12: Create Observability Plan (docs/thor_observability_plan.md)
- [ ] TODO 10 — Phase 13: Create Integration Readiness Review (docs/thor_integration_readiness.md)
- [ ] TODO 11 — Phase 14: Document Manual Production Integration Steps and Add to thor_manual_tasks.md
...
**Status:**
- [ ] Create `mcp/servers/search/` directory structure (server.py, pyproject.toml, README.md, tests/, examples/)
- [ ] Implement `server.py` with MCP stdio transport and three tool handlers
- [ ] Implement SearXNG client with timeout, result limiting, compact output formatting
- [ ] Create `pyproject.toml` with `mcp` dependency and project metadata
- [ ] Write `README.md` documenting the server, tools, configuration, and usage
- [ ] Write `tests/test_search.py` with mocked HTTP tests for all three tools
- [ ] Create `examples/` with a usage example script
- [ ] Run `python -c "import ast; ast.parse(open('server.py').read())"` to verify syntax
...
**Status:**
- [ ] Create `mcp/servers/knowledge/` directory structure (server.py, pyproject.toml, README.md, tests/)
- [ ] Implement `server.py` with MCP stdio transport and four tool handlers
- [ ] Implement Qdrant client with collection allowlist enforcement, read-only operations
- [ ] Create `pyproject.toml` with `mcp` and `qdrant-client` dependencies
- [ ] Write `README.md` documenting the server, tools, allowlist, configuration
- [ ] Write `tests/test_knowledge.py` with mocked HTTP tests for all tools and allowlist enforcement
- [ ] Run syntax check on server.py
...
**Status:**
- [ ] Create `skills/runner/` directory structure (main.py, pyproject.toml, README.md)
- [ ] Implement `main.py` with FastAPI app on port 8091, all three endpoints
- [ ] Implement job model with all status values, artifact path, logging
- [ ] Implement dry-run mode that logs without executing
- [ ] Implement approval gate support (awaiting_approval status)
- [ ] Add tool bundle and model alias declaration fields to job model
- [ ] Create `pyproject.toml` with `fastapi`, `uvicorn`, and dependencies
- [ ] Write `README.md` documenting endpoints, configuration, and dev usage
- [ ] Run syntax check and verify FastAPI imports resolve
...
**Status:**
- [ ] Create `skills/siri_ask/` directory structure (skill.py, skill.yml, README.md)
- [ ] Implement `skill.py` with siri_ask execution logic (chat with model, short response)
- [ ] Create `skill.yml` manifest with all required fields per the architecture doc
- [ ] Write `README.md` documenting the skill, inputs, outputs, and constraints
- [ ] Enforce strict timeouts (30s max runtime)
- [ ] Ensure no broad tools, no admin writes
- [ ] Run syntax check
...
**Status:**
- [ ] Create `skills/deep_research/` directory structure (skill.py, skill.yml, README.md)
- [ ] Implement `skill.py` with multi-step research workflow (search → collect sources → synthesize report)
- [ ] Create `skill.yml` manifest with all required fields
- [ ] Write `README.md` documenting the skill, research process, and output format
- [ ] Implement cited markdown report generation with source list
- [ ] Implement artifact saving to `/home/chuck/data/media/research_reports/`
- [ ] Enforce source limits and runtime limits
- [ ] Run syntax check
...
**Status:**
- [ ] Create `skills/presentation_build/` directory structure (skill.py, skill.yml, README.md)
- [ ] Implement `skill.py` with presentation generation workflow (model content gen → Presenton API call → artifact save)
- [ ] Create `skill.yml` manifest with all required fields
- [ ] Write `README.md` documenting skill, Presenton integration, and LAN-only constraint
- [ ] Document Presenton integration: LAN-only UI, internal skill calls, remote via Siri/skill path
- [ ] Use `PRESENTON_URL` env var for configurable endpoint
- [ ] Run syntax check
